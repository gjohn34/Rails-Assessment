...
When an application is deployed to Heroku, that application's code is run on a virtual machine called a 'dyno', the more dynos an application has the more instances of that application are available for the Routers to make requests. ie a high volume application would require more dynos so as to not create hangs on requests. Each one of these dynos are isolated from each other to prevent one fault to cascade to others, and represents 512MB of physical RAM.
Once the application is up and running, it is credited 750 dyno/hours per month which is the amount of 'free time' available for a user's application to handle requests, applications with a single dyno only is idled until use so as to allocate these dynos to other applications. After an application has been allocated a dyno it is capable to receive requests from the Heroku Router.
...
Heroku allows you to host your application on their private servers instead of users having to create and configure their own.
...
Materialize is a gem that provides styling options for developers so less time is spent styling and more time focusing on the logic of an application. Users have the option of specialising the layout by changing layout class names to the ones provided by Materialize.
...
ActiveStorage is a rails default system of saving images/videos uploaded by users or generated by the system that are saved within Rails and compressed as slugs of     metadata that can be recalled back onto the view. In our application, our ActiveStorage data is stored remotely within Cloudinary’s servers.
...
Over 30 years of continuous improvements
Runs on all major operating systems
ACID-compliant and conforms to 160/179 of the mandatory features
Many powerful add-ons and extensions for more control over a project.
...
Active Storage and Cloudinary- As our application will be receiving user attachments from uploads we will be using Active Storage to save slugs of attachments and Cloudinary to host these images on a 3rd party server
...
After two users are matched together then they can message between each other until one of the users un-match from the other.
...
In our project plan, we organised tasks based on individual strengths and delegated small chunks of tasks to be implemented and completed within an appropriate time. Larger features were broken down and implemented in smaller steps.
...
In a perfect world, these tests would have functioned properly so we could spend less time testing and more time building.
